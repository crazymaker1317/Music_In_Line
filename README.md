...
├── utils/
│   ├── __init__.py
│   ├── visualization.py    # 선 변환 결과 시각화, 피아노 롤 (matplotlib)
...
- [x] 음표 배치가 피아노 롤 시각화로 표시된다
- [x] 생성된 음악을 브라우저에서 바로 재생할 수 있다
- [x] 역방향(오른쪽→왼쪽) 포인트가 자동 제거된다
- [x] 캔버스가 직사각형(800×400)으로 표시된다
- [x] 한글 폰트가 있으면 한글, 없으면 영문 라벨이 표시된다
- [x] 단위 테스트가 통과한다
...
- **악보 시각화**: `music21` 기반 악보 렌더링은 프로토타입에서 생략하였으며, 피아노 롤과 텍스트 기반 음표 정보를 제공합니다.
- **음악적 완성도**: 기초 알고리즘으로 인해 음악적으로 자연스럽지 않을 수 있습니다.
- **오디오 품질**: 사인파 합성을 사용하므로 실제 악기 소리와는 차이가 있습니다.
- **한글 폰트**: matplotlib 시각화에서 한글 폰트가 없는 환경에서는 영문 라벨이 표시됩니다.
